...
$$
\mathscr{H}^{\dagger} = \mathscr{H} + \phi_{N+1}(p,q) \lambda_{N+1}(t) +  \sum_{i=1}^{N} \phi_{N+1+i}(p,q) \lambda_{N+1+i}(t)
$$

其中，$\phi_{N+1}(p, q) = p_{N+1} , \phi_{N+1+i}(p, q) = p_{N+1+i}$。
...
容易注意到只要将我们的拉氏量中的 $L(q)$ 换成最优控制中的 $-C(x,u)$，我们立刻得到与庞特里亚金定理几乎相同的表述。
