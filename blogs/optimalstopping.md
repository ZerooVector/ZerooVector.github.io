...
#### 问题描述
玩家面前有一列苹果，每个苹果的质量是$m_i$。玩家只能依次看到第$i=1,2,3,\cdots,n$个苹果的重量。在玩家获知第$i$个苹果的重量后，玩家可以选择拿取这个苹果，或者继续向前走；玩家一旦获知第$i$个苹果的重量，就不能拿取编号小于$i$的苹果。每个玩家在拿取两个苹果后游戏结束，两个苹果中质量最大的那个是玩家的最终得分。
